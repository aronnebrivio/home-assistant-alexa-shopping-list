...
def _config_path():
    return os.environ.get(
        "ASL_CONFIG_PATH", os.path.dirname(os.path.realpath(__file__))
    )
...
async def _cmd_config_valid():
    return os.path.exists(os.path.join(_config_path(), "config.json")), None
...
def _start_alexa():
    ...
    if alexa_running is False:
        alexa = AlexaShoppingList(
            _get_config_value("amazon_url", "amazon.co.uk"), _config_path()
        )
        alexa_running = True
...
async def _cmd_is_authenticated():
    ...
    if instance.requires_login() is True:
        print("\nAuthenticated: No")
        result = False, None
    else:
        print("\nAuthenticated: Yes")
...
async def _process_command(websocket, path):
    ...
    async for message in websocket:
        ...
        if len(results) == 2:
            response = {"result": results[0], "error": results[1]}
        else:
