...
class AlexaShoppingList:
    ...
    def _setup_driver(self):
        ...
        if len(self.driver.find_elements(By.ID, "nav-backup-backup")) > 0:
            self.driver.find_element(By.CLASS_NAME, "nav-bb-right").find_element(
                By.LINK_TEXT, "Your Account"
            ).click()
            time.sleep(5)

        if (
            len(self.driver.find_elements(By.CLASS_NAME, "nav-action-signin-button"))
            > 0
        ):
    ...
    def _selenium_get(
        self, url: str, wait_for_element: tuple = None, wait_for_page_load: bool = False
    ):
        self.driver.get(url)
    ...
    def submit_mfa(self, code: str):
        ...
        time.sleep(5)
        if self.login_requires_mfa() is False:
    ...
    def login(self, email: str, password: str):
        self._selenium_get(
            "https://www." + self.amazon_url, (By.ID, "nav-link-accountList")
        )
    ...
    def _ensure_driver_is_on_alexa_list(self, refresh: bool = False):
        list_url = (
            "https://www."
            + self.amazon_url
            + "/alexaquantum/sp/alexaShoppingList?ref=nav_asl"
        )
    ...
    def _get_alexa_list_item_element(self, item: str):
        ...
        for container in list_container.find_elements(By.CLASS_NAME, "inner"):
            if (
                container.find_element(By.CLASS_NAME, "item-title").get_attribute(
                    "innerText"
                )
                == item
            ):
    ...
    def add_alexa_list_item(self, item: str):
        ...
        self.driver.find_element(By.CLASS_NAME, "list-header").find_element(
            By.CLASS_NAME, "add-symbol"
        ).click()

        textfield = (
            self.driver.find_element(By.CLASS_NAME, "list-header")
            .find_element(By.CLASS_NAME, "input-box")
        ...
        submit = (
            self.driver.find_element(By.CLASS_NAME, "list-header")
            .find_element(By.CLASS_NAME, "add-to-list")
            .find_element(By.TAG_NAME, "button")
        )
        submit.click()

        self.driver.find_element(By.CLASS_NAME, "list-header").find_element(
            By.CLASS_NAME, "cancel-input"
        ).click()
        time.sleep(1)
    ...
    def update_alexa_list_item(self, old: str, new: str):
        ...
        element.find_element(By.CLASS_NAME, "item-actions-1").find_element(
            By.TAG_NAME, "button"
        ).click()

        textfield = element.find_element(By.CLASS_NAME, "input-box").find_element(
            By.TAG_NAME, "input"
        )
        textfield.clear()
        textfield.send_keys(new)

        element.find_element(By.CLASS_NAME, "item-actions-2").find_element(
            By.TAG_NAME, "button"
        ).click()
        time.sleep(1)
